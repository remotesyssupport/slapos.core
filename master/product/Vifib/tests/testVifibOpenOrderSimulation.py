...
class TestVifibOpenOrderSimulation(TestVifibSlapWebServiceMixin):
  ...
  def stepCheckSimulationMovement(self, sequence, **kw):
    ...
    # Check periodicity of the open sale order line (like on alarm)
    self.assertEquals("XXX", open_order_line.getPeriodicityMinuteFrequency())
    self.assertEquals("XXX", open_order_line.getPeriodicityMinuteList())
    self.assertEquals("XXX", open_order_line.getPeriodicityHourFrequency())
    self.assertEquals("XXX", open_order_line.getPeriodicityHourList())
    self.assertEquals("XXX", open_order_line.getPeriodicityDayFrequency())
    self.assertEquals("XXX", open_order_line.getPeriodicityDayList())
    self.assertEquals("XXX", open_order_line.getPeriodicityMonthFrequency())
    self.assertEquals("XXX", open_order_line.getPeriodicityMonthList())
    self.assertEquals("XXX", open_order_line.getPeriodicityMonthDayList())
    self.assertEquals("XXX", open_order_line.getPeriodicityWeekFrequency())
    self.assertEquals("XXX", open_order_line.getPeriodicityWeekList())
    self.assertEquals("XXX", open_order_line.getPeriodicityWeekDayList())
    
    # Check start/stop date
    self.assertEquals("XXX", open_order_line.getPeriodicityStopDate())
    self.assertEquals("XXX", open_order_line.getStartDate())

    # Calculate the list of time frames
    expected_time_frame_list = []

    # Check that simulation is created by the periodicity
    self.assertEquals(len(expected_time_frame), 
                      len(applied_rule.contentValues()))

    # Check the list of expected simulation
    for expected_time_frame in expected_time_frame_list:
      simulation_movement = self.portal.portal_catalog.getResultValue(
          parent_uid=applied_rule.getUid(),
          portal_type="Simulation Movement",
          start_date=expected_time_frame[0],
          stop_date=expected_time_frame[1],
          )
